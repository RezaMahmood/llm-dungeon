...
| [`frontend-deploy.yml`](frontend-deploy.yml) | **Frontend Deploy** | `workflow_dispatch` (`version` input); also `workflow_call` (used by `full-build-deploy.yml`) | CD. Resolves the requested (or latest-at-execution-time) version, deploys its cached artifact as-is — building it first only if it isn't cached yet. No approval gate. |
| [`backend-deploy.yml`](backend-deploy.yml) | **Backend Deploy** | `workflow_dispatch` (`version` input); also `workflow_call` (used by `full-build-deploy.yml`) | CD. Same pattern as frontend, for the Azure Functions backend. No approval gate. Deploy uses `remote-build: true` — Azure Flex Consumption doesn't support a pre-built/vendored Python package (confirmed empirically in production: `remote-build: false` deployed "successfully" but loaded zero functions). The exact tested-and-versioned source tree is still what's deployed; only the "no build step at deploy time" part yields to this platform constraint. |
| [`infrastructure-deploy.yml`](infrastructure-deploy.yml) | **Infrastructure Deploy** | `workflow_dispatch`, no inputs; also `workflow_call` (used by `full-build-deploy.yml`) | CD. No versioning, no persistent artifact — always operates against the current state of `main`. Job graph: `validate-and-test` → `plan` → `apply`. `apply` targets the `production-infra` environment, which requires human approval before it applies (see below), and applies the exact plan `plan` produced in the same run — never re-planning. |
| [`full-build-deploy.yml`](full-build-deploy.yml) | **Full Build & Deploy** | `workflow_dispatch` only (`mode`, `backend_version`, `frontend_version` inputs) | CD orchestrator. Calls `infrastructure-deploy.yml`, then `backend-deploy.yml`, then `frontend-deploy.yml` in sequence via `workflow_call` — no duplicated logic, each keeps its own guarantees (infra's approval gate, backend/frontend's build-on-demand-on-cache-miss). `mode: "refresh everything"` deploys latest-at-execution-time for both components; `mode: "specific versions"` deploys the given `backend_version`/`frontend_version` (blank still falls back to latest for that component). |
| [`app-deploy.yml`](app-deploy.yml) | **App Deploy (Backend + Frontend)** | `workflow_dispatch` only (`mode`, `backend_version`, `frontend_version` inputs) | CD orchestrator. Same pattern as `full-build-deploy.yml` but never touches infrastructure — calls only `backend-deploy.yml` then `frontend-deploy.yml` via `workflow_call`. `mode: "refresh both"` deploys latest-at-execution-time for both components; `mode: "specific versions"` deploys the given `backend_version`/`frontend_version` (blank still falls back to latest for that component). |
...
| [`functions-always-ready-schedule.yml`](functions-always-ready-schedule.yml) | **Functions Always-Ready Schedule** | `schedule` (hourly at :00 and :50), `workflow_dispatch` (`mode`: auto/on/off) | Scales the Function App's `http` always-ready instance count between `terraform.tfvars`'s `functions_always_ready_instance_count` and 0, so warm instances are only billed during development hours (Europe/London: weekdays 17:00-23:00, weekends 10:00-22:00). Window logic lives in `scripts/functions-always-ready-window.sh`, unit-tested by `workflow-structure-test.yml`. |
