...
function testUserStory3() {
  ...
  // Regression guard (found live during real quickstart.md Scenario 4
  // re-validation, 2026-09-03, in two rounds): GitHub Actions' default job
  // condition (success()) is evaluated against the entire upstream
  // dependency graph, not just a job's own `needs` list. ensure-artifact
  // uses `if: always()` to survive build-on-demand being skipped (the
  // normal, expected outcome for an explicit already-cached version) —
  // but deploy, which only `needs: ensure-artifact`, was still
  // transitively skipped whenever build-on-demand was skipped, even
  // though ensure-artifact succeeded. The run reported overall success
  // while silently deploying nothing. Round one's fix (a bare
  // `needs.ensure-artifact.result == 'success'`, no always()) still did
  // not work, confirmed live: GitHub Actions implicitly ANDs *any* custom
  // `if:` with success() — the very same transitive check — unless the
  // expression itself calls a status-check function. Both the always()
  // and the exact 'success' comparison are required.
  for (const name of ["frontend-deploy.yml", "backend-deploy.yml"]) {
    const deployJob = loadWorkflow(name).jobs.deploy;
    check(
