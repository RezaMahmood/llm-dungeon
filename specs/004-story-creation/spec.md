...
These surfaced while the requesting user manually verified the implemented wizard against this spec. They are **not resolved** — implementation should not change until the user has refined the intended flow — and T033 (user-verified acceptance, `tasks.md`) is blocked on them.

- **Cover image URL — no defined meaning**: `data-model.md`'s `coverImageUrl` field (Name & cover step) has never had its expected content specified — is it a direct link to an externally-hosted image, an uploaded/managed asset reference, or something else? Neither this spec nor the design mockup says. Needs a decision before the field can be considered correctly implemented.
- **Auto-generation can pre-empt the wizard before the administrator has visited all four steps**: FR-004 requires the system to generate and persist the story automatically "once it is complete," with no manual save step. As implemented, "complete" only requires World & setting's fields (world prompt, ≥1 character type, ≥1 completion criterion, per FR-008) — tone/reading level (step 03) and session length (step 04) are optional and not required for completion. So if an administrator fills in World & setting efficiently in one exchange, the wizard can jump straight from that single AI response to the finished, generated-story screen — replacing the whole wizard view with no warning, and without the administrator ever having reached steps 03/04. The requesting user experienced this as "the AI response loads a new page — this is not part of the flow." FR-002's "reachable in any order" framing did not anticipate generation potentially firing before every step is even visited once. The intended conversational/generation flow needs to be redesigned by the requesting user (placeholder here pending that refinement) — options include confirming before generating, or requiring all four steps to be visited at least once regardless of which fields are strictly required.
