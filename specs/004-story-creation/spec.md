...
## Open Questions — Resolved by Session 2026-08-30 Clarifications

These surfaced while the requesting user manually verified the originally-implemented (auto-generate-on-completeness) wizard against this spec. Both are now resolved by the Session 2026-08-30 Clarifications above and the corresponding redesign (explicit Save/Abandon/Finished, local-storage draft, blob-stored cover image); the historical record is kept below rather than deleted.

- **Cover image URL — no defined meaning**: resolved by the Session 2026-08-30 Clarifications ("What does Tab 01 ... collect, and how is the cover image handled?") and FR-009 — the cover image is a file uploaded from the administrator's device, written to blob storage on Save, with the story record storing a reference (URL) to the blob, not a raw externally-hosted link.
- **Auto-generation can pre-empt the wizard before the administrator has visited all four steps**: resolved by the Session 2026-08-30 Clarifications ("Is the story persisted automatically...?") and FR-004 — there is no more automatic generation-triggered persistence at all; the story is written to the database only on an explicit Save, available from any tab at any time, so the previous "jumps to a finished screen mid-conversation" failure mode no longer exists.
