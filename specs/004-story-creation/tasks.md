...
- [ ] T013 [US1] Rewrite `src/backend/api/manage/stories.py`: replace the `create_story`/`list_stories` placeholders with `create_draft`, `get_draft`, `patch_draft`, `post_message`, `list_stories`, `get_story` handlers per contracts/api.md, each gated by `authorize_admin` (depends on T009, T010)
