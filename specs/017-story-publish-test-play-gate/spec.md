...
**Split**: This spec depends on `010-story-test-play` for the Test Play Exchange concept it gates on, and on `005-story-publishing-done` for the publish action it blocks.
...
**Why this priority**: This is what makes test play meaningful rather than optional busywork. It depends on both `010-story-test-play` (test play exists) and `005-story-publishing-done` (a publish action exists to gate).
