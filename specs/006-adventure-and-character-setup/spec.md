...
- Every published adventure has at least one cast character defined; `005-story-publishing-done` is responsible for blocking publication of an adventure with zero of them. This no longer bears on the setup flow, which stopped offering the roster as a choice (`032-story-archetypes-player-avatar` FR-002) — the player writes their own avatar description instead, so a roster's size cannot leave setup with nothing to show.
