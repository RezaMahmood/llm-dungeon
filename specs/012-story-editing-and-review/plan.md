...
- **Traceability**: constitution v2.3.0 adds the **Administrator — stories & configuration** screen contract, covering this feature's two screens (the story list with status and publish/unpublish entry point, and the read-only configuration viewer with its download). The deferral covers styling only — it relaxes nothing this spec requires of the two screens, and none of the constitution's design-token, interaction-state or accessibility bar. Supersedes: constitution v10.0.0 (issue #340) withdrew the Screen contracts section outright, so there is no longer a traceability requirement to meet and no second statement of these screens' affordances; this spec is their sole source, and `specs/designs/README.md` records that they ship without a prototype.
