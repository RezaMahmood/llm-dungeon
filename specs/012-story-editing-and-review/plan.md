...
- **Traceability**: constitution v2.3.0 adds the **Administrator — stories & configuration** screen contract, covering this feature's two screens (the story list with status and publish/unpublish entry point, and the read-only configuration viewer with its download). That section now also states explicitly that a screen may exist without a prototype, which is exactly FR-012's case; `specs/designs/README.md` points at it. The deferral covers styling only — it relaxes nothing this spec requires of the two screens, and none of the constitution's design-token, interaction-state or accessibility bar. Supersedes: the contract's own enumerated affordances, withdrawn by constitution v10.0.0 (issue #340), which now names this spec as the owner instead.
