...
**Decision**: The file carries exactly `id` (optional), `name`, `coverImageUrl`, `tone`,
`readingLevel`, `sessionLengthMinutes`, `chapters`, `worldPrompt`, `rules`, `characterTypes`,
`completionCriteria`. It never carries `published`, `lastPublishedAt`, `createdBy`,
`createdAt`, `contentUpdatedAt`, `lastUpdatedBy`, `lastTestPlayedAt`, `contentVersion`,
`entityType`, or `narrativeGuidance`.
...
store, out of a file that leaves it). `narrativeGuidance` is excluded because it is
**derived**, not authored — it is generated by the LLM from the authored fields (see §5) and
is not in FR-004's list.

**Alternatives considered**:
- *Include `narrativeGuidance` so a round-trip is loss-free* — rejected: it is not authored
  content, it would let a hand-edited file desynchronise the narrative guidance from the world
  it describes, and FR-004's list is exhaustive.
...
## 5. `narrativeGuidance` on save and on import

**Decision**: Regenerate `narrativeGuidance` from the resulting configuration on **every**
content write — wizard edit save, id-matched overwrite import, and id-less new-story import —
by reusing `LLMService.generate_story_config`, exactly as creation does. A generation failure
fails the whole save (502 `generation_failed` / 429 `rate_limited`) and leaves the story
unchanged.

**Rationale**: `narrativeGuidance` is derived from `worldPrompt`, `rules`, `characterTypes`,
and `completionCriteria`. Keeping the old guidance after the world it describes was replaced
would leave gameplay narrating the pre-edit story, which is a worse failure than a rejected
save. Reusing the creation path also keeps one generation prompt and one telemetry shape
(Principle VI).
