...
| 4 | Download contains `id` + authored fields and **no** system-managed fields (FR-004) | `test_story_config_file.py` | `published`, timestamps, `createdBy`/`lastUpdatedBy`, `lastTestPlayedAt`, `narrativeGuidance` are absent; `id` is present. |
