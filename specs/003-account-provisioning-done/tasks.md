...
- [X] T050 Run quickstart.md's 7 validation scenarios end-to-end (locally or against a deployed environment) and confirm each passes — completed 2026-08-30 against the deployed environment (see issue #32); Scenario 4's domain-existence gap noted below is accepted as-designed, not a failure
...
- [X] T070 Add 1-2 new scenarios to `quickstart.md` covering Entra-invite-on-grant and the removal flow, and re-run T050's live validation once T057-T069 are implemented — depends on T059, T060 — scenarios added 2026-08-30, live re-run completed 2026-08-30 (see issue #32)
- [X] T071 **User-verified acceptance** (Constitution Principle IX, NON-NEGOTIABLE): the requesting user or product owner — not the implementing agent — signs in as an Administrator against the real deployed environment (or the most representative environment available) and manually runs quickstart.md's full scenario set end-to-end, including the Entra-guest-invite-on-grant and account-removal scenarios added by T070 (confirming a newly granted account with no prior tenant presence can sign in, and a removed account cannot). This task is not complete until that confirmation is given; a passing T070/T050 agent-run validation does not satisfy it — depends on T070 — **confirmed by the requesting user 2026-08-30** via the live UI against the deployed environment: Scenarios 3, 5, 6, 7, 8, 9 passed; Scenario 4's malformed-email rejection passed but its domain-existence gap is accepted as designed behavior (RFC 5322 grammar only, per FR-005/research.md — logged as a future enhancement, not a defect); self-removal's `400 self_removal` rejection was not exercised live (the UI correctly has no self-remove action to trigger it) but remains covered by `test_admin_accounts_endpoint.py` (T068)
