...
- An administrator deletes a story while they themselves (or another administrator) have an active test-play session against it: the test-play session is not persisted player data (see `010-story-test-play`) and simply becomes unusable; no player-facing notification applies since test play is administrator-only.
