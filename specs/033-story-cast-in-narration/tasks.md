...
- [X] T010 CRITICAL: Add a test asserting a story whose roster has a single `CharacterType`
...
- [X] T011 CRITICAL: Strengthen the two weak assertions in
...
- [X] T012 In `src/backend/services/prompts/gameplay_turn_system_prompt.txt`, name the cast
      ...
      (partial)
- [X] T013 Run `pytest tests/unit/test_llm_service.py -k gameplay_turn_prompt` and then the full
