...
## Decision 3: Use GitHub Releases (tag + release + attached asset) as the artifact cache, for all three components

**Decision**: Continue frontend/backend's existing pattern — `semantic-release` (via `semantic-release-monorepo`) computes each component's next SemVer from path-scoped Conventional-Commit-typed changes, creates a git tag (`frontend-v*` / `backend-v*`) and GitHub Release, and the build job attaches the deployable artifact as a release asset. Extend the identical pattern to infrastructure: a new `infrastructure/.releaserc.json` path-scoped to `infrastructure/**`, tag prefix `infrastructure-v*`, with the release asset being the saved Terraform plan (binary `tfplan` + human-readable plan text) plus a `VERSION` file, rather than a compiled application bundle.
...
## Decision 9: Infrastructure gets its own SemVer version history (independent of frontend/backend), starting at `0.1.0`

**Decision**: `infrastructure/.releaserc.json` is a new `semantic-release` config, path-scoped to `infrastructure/**`, producing independently-numbered `infrastructure-v*` tags/releases — mirroring frontend's and backend's existing configs and their `0.1.0` starting baseline (per this spec's predecessor's clarification, reused here for consistency since infrastructure has likewise never shipped a stable `1.0`).

**Rationale**: FR-005 requires each component to have its own independent version history; the earlier (superseded) version of this spec had explicitly excluded infrastructure from versioning, but the current spec's User Stories 3-5 and Assumptions explicitly include infrastructure in the same versioned/cached/deployable pattern as frontend and backend.

**Alternatives considered**: *Version infrastructure off the same commit-type analysis but without a starting baseline decision (defaulting to semantic-release's own `1.0.0` first-release default)*: rejected for consistency — frontend/backend already deliberately chose `0.1.0` over `1.0.0` for the same "nothing has shipped a stable 1.0 yet" reason; applying a different rule to infrastructure alone would be an unexplained inconsistency.
