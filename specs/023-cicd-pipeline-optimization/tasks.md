...
- [ ] T025 [US4] Add the new check's job name to `main`'s required-status-checks branch protection list (repo admin action via GitHub settings or `gh api`), and document the requirement in `.github/workflows/README.md` — files: GitHub branch protection settings (not a repo file), `.github/workflows/README.md`. Depends on T024. **Partially done**: README documented (see "One-time rollout step: require the new checks"); the actual branch-protection change is deliberately deferred — it's a live setting on the real repo that would immediately block all merges until this feature's own PR has merged and these checks exist as real check runs, so it must happen post-merge, by a repo admin, not during implementation.
