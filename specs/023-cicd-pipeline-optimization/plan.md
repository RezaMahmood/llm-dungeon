...
**Post-merge amendment (2026-09-02)**: After this plan's design merged (PR #160), the requesting user reversed its infrastructure-versioning decision — infrastructure no longer builds/versions/caches an artifact; its deploy always validates/tests/plans fresh. This plan document is kept as the historical record of the original (still largely accurate for frontend/backend) design; see spec.md's Clarifications Amendment, research.md's amendment to Decision 9, and data-model.md/contracts/workflow-interfaces.md for the corrected current behavior.
