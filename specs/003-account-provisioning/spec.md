...
6. **Given** a signed-in Administrator grants access to an email with no existing presence in the application's Entra ID tenant, **When** the entry is created, **Then** that email is also added to the tenant as an external guest user, so the account can complete Microsoft sign-in without any manual directory step outside the application.

---

### User Story 3 - Administrator Removes a Player or Administrator's Access (Priority: P3)

A signed-in Administrator uses the in-app interface to remove an existing provisioned account entry by email, revoking that account's access entirely — except an Administrator cannot remove their own entry or the seed administrator's entry.

**Why this priority**: Complements User Story 2 — access that was granted must also be revocable, without which mistakes or accounts that should no longer have access can only be corrected by a direct database edit. This was an explicit, deliberate scope boundary of the original spec ("a future capability") that is now being addressed.

**Independent Test**: Signed in as an administrator, remove a previously granted Player (or Administrator) entry by email, and verify that account can no longer sign in; separately, verify attempting to remove the signed-in administrator's own entry or the seed administrator's entry is rejected.

**Acceptance Scenarios**:

1. **Given** a signed-in Administrator and an existing provisioned account entry for another user, **When** the Administrator removes that entry, **Then** the entry is deleted from the allow list and that email's corresponding guest user is removed from the application's Entra ID tenant.
2. **Given** the removed account attempts to sign in after removal, **When** they present their Microsoft credentials, **Then** access is denied exactly as for a never-provisioned account.
3. **Given** a signed-in Administrator, **When** they attempt to remove their own provisioned account entry, **Then** the system rejects the removal and indicates administrators cannot remove themselves.
4. **Given** a signed-in Administrator, **When** they attempt to remove the seed administrator's entry (identified by the deployment-configured seed email), **Then** the system rejects the removal regardless of who is signed in, so the system can never end up with zero administrators via this path.
...
- **Known gap (2026-08-30)**: prior to this amendment, an Administrator granting access to a Microsoft account with no existing presence in the application's Entra ID tenant would create an allow-list entry that could never actually sign in (`AADSTS50020`), since tenant membership is a prerequisite to sign-in that this feature did not establish — see FR-011.
- An administrator removes a provisioned account entry that has no bound Microsoft object identifier yet (the account has never signed in): removal still succeeds, deleting the entry and, if present, the corresponding Entra tenant guest user.
- An administrator attempts to remove an email that has no provisioned entry at all: the system rejects the request rather than silently succeeding, since there is nothing to remove.
- An administrator removes a provisioned account entry whose email was never actually added as an Entra tenant guest (e.g., it already had some other pre-existing tenant presence): removal of the allow-list entry still succeeds even if no matching guest user is found in the tenant.
...
- A sign-in presents an email that matches a provisioned entry but a Microsoft object identifier that does not match that entry's already-bound object identifier: access is denied, the same as any other non-matching sign-in. Clearing or replacing an existing binding (e.g., because a user's Microsoft account was recreated) is not supported by this feature and would need a future capability.
...
- **FR-010**: Each distinct provisioning outcome (initial seed present at first run, successful new-entry add for each role combination, successful role-merge on an already-provisioned email, rejected submission with no role selected, rejected malformed email, first sign-in binding a Microsoft object identifier to a provisioned entry, successful subsequent sign-in with a matching bound object identifier, denied sign-in when the presented object identifier does not match the entry's bound object identifier, an Entra ID guest invite issued on a new grant, an invite skipped when the email is already a tenant member, a successful removal deleting both the allow-list entry and its Entra tenant guest, a rejected self-removal, and a rejected removal of the seed administrator's entry) MUST have a corresponding automated test verifying its expected behavior.
- **FR-011**: When an Administrator grants a provisioned account entry (new or role-merge) for an email with no existing presence in the application's Entra ID tenant, the system MUST add that email to the tenant as an external guest user, so the account can complete sign-in without a separate manual directory step.
- **FR-012**: System MUST provide an interface, reachable only by a signed-in user holding the Administrator capability, for removing an existing provisioned account entry by email, and MUST reject an attempt to remove the signed-in Administrator's own entry or the seed administrator's entry.
- **FR-013**: When a provisioned account entry is removed, the system MUST also remove that email's corresponding Entra ID tenant guest user, not only the allow-list entry, so the removed account can no longer sign in.

### Key Entities

- **Provisioned Account Entry**: An email address together with the capability role(s) (Player and/or Administrator) granted to it, and the Microsoft object identifier bound to it once its first successful sign-in has occurred (absent until then). This is the concrete record behind the Allow-List Entry and Capability Role concepts described in `002-login-and-access-control` — a signed-in Microsoft account is matched against these entries by email on first sign-in (binding its object identifier), and by that bound object identifier on every sign-in thereafter, to determine both whether it may sign in at all and which capabilities it holds. Viewing the list of these entries is specified in `014-account-listing`. Its lifecycle now includes removal (User Story 3), and its email corresponds 1:1 to an Entra ID guest user once granted (FR-011).
...
- **SC-007**: 100% of newly granted accounts with no prior Entra ID tenant presence can complete Microsoft sign-in on their very next attempt, with no manual tenant-invite step performed outside the application.
- **SC-008**: 100% of removed accounts (other than the acting Administrator's own account or the seed administrator) can no longer sign in immediately after removal, and no longer appear in the application's Entra ID tenant as a guest user, in testing.
...
- Clearing or replacing an entry's bound object identifier (e.g., because the underlying Microsoft account was deleted and recreated) is out of scope for this feature; a future capability would need to address rebinding.
...
- Both the Entra ID guest invite (FR-011) and the Entra ID guest removal (FR-013) are performed by the backend on the Administrator's behalf via Microsoft Graph; this requires the backend's own service identity to hold a Graph API permission grant, which is an infrastructure/deployment concern (tracked in tasks.md), not an action the Administrator performs directly.
