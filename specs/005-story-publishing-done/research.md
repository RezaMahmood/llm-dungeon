...
## 5. Reachability from the administrator story list (FR-010)

**Decision** (revised 2026-09-06): This feature builds **both** of FR-010's entry points — the `publish`/`unpublish` endpoints with the wizard's `StepPublish.jsx` (Phase 3), and the per-row publish/unpublish action in the administrator story list (Phase 5). The list is not built from scratch: it already exists at `src/frontend/src/pages/AdminPage.jsx`, rendering each story's name and published status, so Phase 5 adds a control to an existing screen rather than standing up a placeholder one. Both callers share one frontend action and one backend gate check, satisfying FR-010's "both entry points enforce the identical FR-008 precondition" by construction.

**Rationale**: The original decision below deferred the list entry point to `012-story-editing-and-review` on the premise that no story list existed. That premise was wrong — the list shipped with `004-story-creation-done` — and `012` is not yet started, so deferring would have shipped this feature with FR-010 unmet and no way to publish an already-generated story outside the creation wizard (exactly the friction recorded in T017's 2026-09-05 validation note). `012` extends this same list with its editing and review capabilities; it does not introduce a second story list or a second publish path, so the duplication risk the original decision guarded against does not arise.

**Superseded decision (2026-08-30)**: *"This plan builds the `publish`/`unpublish` endpoints and the wizard's `StepPublish.jsx` entry point only. It does not build a story-list page — `012`'s spec note confirms that screen 'is not yet part of the design reference prototype' and is `012`'s own scope."* Retained for traceability; no longer in force.
