...
- **FR-011**: Ongoing gameplay LLM usage generated by real players playing a published story is out of scope for this per-story creation total (see Assumptions); this feature MUST NOT be interpreted as replacing the project's existing aggregate AI-spend telemetry/dashboards. Real-player gameplay usage is tracked instead per FR-012 through FR-018 below, and MUST NOT be added into the story's cumulative creation total from FR-002. Admin test-play usage is different: per FR-001, it IS added into that cumulative creation total, in addition to being tracked per FR-012 through FR-018 like any other gameplay session.
...
- **Story**: The existing record representing an authored adventure. Gains one new attribute — its cumulative token total — that accumulates over the story's authoring lifecycle (creation, content edits/regenerations, and admin test plays, per FR-001) and is shown in the stories list. This total is distinct from a real player's gameplay session total, which is never folded into it (FR-011); an admin test-play session's usage, by contrast, counts toward both its own session total and the story's cumulative total.
