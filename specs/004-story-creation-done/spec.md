...
- Q: `008-core-gameplay-done` already defines "Completion Criteria" as a maximum duration, success condition(s), failure condition(s), and an any/all combination rule, and states this shape is authored in `004-story-creation` — should the new completion-criteria field (FR-008) require that same structured shape? → A: Yes — match `008-core-gameplay-done`'s structured shape (optional max duration, one or more success conditions, optional failure conditions, and an any/all rule when more than one condition is defined).
...
- **FR-006**: A newly created story configuration MUST default to unpublished, so it is not visible to players until an administrator explicitly publishes it (see `005-story-publishing-done`).
...
- Making a created story visible to players is a separate, explicit action, not part of this feature (see `005-story-publishing-done`).
