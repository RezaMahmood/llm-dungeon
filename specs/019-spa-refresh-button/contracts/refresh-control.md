...
| Play surface | `/game` (mockup: `03-play.html`) | Placeholder today (008-core-gameplay-done builds real content) | `TitleBar.jsx`'s `trailing-actions` slot (not wired in this feature) | Deferred until 008 gives this screen real data — see plan.md Scale/Scope; `TitleBar` will consume `RefreshContext` the same way `NavBar` does once that lands |
