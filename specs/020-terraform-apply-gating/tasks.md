...
- [ ] T007 [US1] Compare a run from T004/T005 (`apply` = `skipped`) against a run from
  T009 below (`apply` = `waiting`, pending environment review) and confirm, per
  `quickstart.md` Scenario 6, that an engineer can tell "blocked by the automated gate"
  apart from "blocked on manual approval" without opening any other workflow's run.
  Depends on: T004, T005, T009.
...
- [ ] T012 Final user-verified acceptance (Constitution Principle IX): the requesting
  user or product owner reviews the real GitHub Actions runs produced by T004, T008, and
  T010 (or an equivalent live run they trigger themselves) directly in the repository's
  Actions tab, and confirms the gate and approval behavior match spec.md's acceptance
  scenarios before this feature is considered done. This task is not complete until that
  confirmation is given — not merely on the strength of T004–T010 having been executed
  by the implementing agent. Depends on: T004, T008, T010.
...
- T001 must land before T002/T003 are meaningful to verify end-to-end (though T002/T003
  ...
  T001).
- T004, T005, T006 each depend only on T001.
- T007 depends on T004 and T005 (compares their outcomes) and also implicitly on T009
  (needs a `waiting`/approval-pending run to contrast against).
...
- T004, T005, T006 can be run in parallel with each other (independent pushes/dispatches)
  once T001 has landed.
...
2. T004–T007 → gate proven to block correctly and to be visibly distinct from approval
   waiting (User Story 1 complete).
3. T008–T010 → manual approval proven to still hold once the gate passes (User Story 2
   complete).
4. T011–T012 → documentation pointer updated, final human sign-off recorded.
